...
$sqlFile = Join-Path $repoRoot 'sql/postgresql/001_init_schema.sql'
...
$tempContainerFile = '/tmp/001_init_schema.sql'
...
if ($execProcess.ExitCode -ne 0) {
  throw ('PostgreSQL initialization failed with exit code ' + $execProcess.ExitCode + '.')
}

Write-Host ''
Write-Host 'PostgreSQL schema initialization completed successfully.'
