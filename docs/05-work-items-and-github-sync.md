...
- 已完成：阶段顺序确认。
- 已完成：本文件建立并记录实施流程。
- 已完成：阶段 4 的本地种子数据、Keycloak 认证底座、JWT 验签与前端真实登录接入。
- 已完成：阶段 5 需求拆分与接口/逻辑设计文档，见 `docs/11-stage5-business-design-and-api-contract.md`。
- 待执行：初始化 Git、创建 GitHub 远程仓库、首推 main。
- 待执行：阶段 5 的 API 契约固化（统一响应、错误码、OpenAPI 与前端 SDK/封装）。
